...
`background.js` — Manifest V3 service worker. Он загружает только разрешённые прямые VK CDN URL, ограничивает файл 30 МБ и отдаёт его content script порциями. Байты существуют только во временной памяти до добавления текущего файла в ZIP и затем освобождаются. Это позволяет добавить медиа в архив без ослабления CORS страницы и без постоянного кэша на диске.

Успешно загруженные CDN-ответы сохраняются в Cache Storage расширения. Метаданные LRU находятся в `chrome.storage.local`; лимит 500 МБ поддерживается удалением давно не использованных записей. Кэш не синхронизируется и очищается пользователем через popup.
