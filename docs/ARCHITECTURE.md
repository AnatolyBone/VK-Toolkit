...
`dedupe.analyzeCmids()` строит непрерывные сегменты и компактные диапазоны разрывов за линейный проход по отсортированным уникальным CMID. Короткие разрывы считаются отдельными пропусками, крупные помечаются как вероятно несобранные; это эвристика, а не утверждение об удалении сообщений.
